...
| 分類 | 技術 | 用途・理由 |
|------|------|-------------|
| 言語 | Python 3.9+ | データ処理・AIモデル構築・DBアクセスの全てを一貫して実装可能。学習コストとライブラリの豊富さを両立。 |
| データ分析 | pandas / numpy | 統計情報や実行ログの加工・分析処理に使用。処理対象が軽量なため、pandas / numpy を選定。※1 |
| 機械学習 | scikit-learn | 初期段階では回帰モデルや前処理を中心に活用。モデルの保存・再利用も容易。 |
| データ可視化（任意） | matplotlib / seaborn | モデル精度や特徴量分布の確認用途（PoC段階） |
| データベース接続 | psycopg2 | PostgreSQLと安全に連携可能なDBドライバとして採用。 |
| ログ管理 | logging | 実行結果やエラー情報をログ出力。学習用データの蓄積にも利用。 |

※1：クエリ1件あたりの処理対象が小規模（最大100件程度）のため、起動の軽さと成熟したエコシステムを重視して pandas / numpy を選定した。  
...
| 区分 | 内容 |
|------|------|
| OS | Linux（Ubuntu 22.04）想定。macOSも動作対象。 |
| Pythonバージョン | Python 3.9 以上 |
